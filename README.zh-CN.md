...
**1. 会话层** —— 与真实终端源建立连接、收发原始字节。后端包括本地 PTY、Windows
ConPTY、串口、SSH 以及自定义远程流。它只负责如何连接、如何读写原始字节、如何传递
窗口尺寸变化，以及如何处理中断、关闭、错误与重连；不做 VT 解析，也不维护屏幕状态。

**2. 协议层** —— 将终端协议解析为 VT/ANSI 语义：UTF-8 解码、C0/C1 控制字符、
ESC/CSI/OSC/DCS/APC/PM 序列、SGR 样式属性、光标移动、擦除与插入删除。

**3. 状态模型层** —— 持有终端状态：cell buffer、滚动历史、光标、模式、选区、
resize 重排与脏区跟踪。正确性在这一层决定，且可完全无头测试。

**4. 渲染与交互层** —— 把核心状态投影成前端可绘制的数据，并把输入编码为协议字节。
Qt Quick 与 QWidget 两个前端都建立在这一层之上，而不是各自重新实现核心。
